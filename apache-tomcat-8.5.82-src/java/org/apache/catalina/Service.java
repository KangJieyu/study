 ...
 *
 * 一个 Service 是一组或多组的 Connectors，共享一个单独的 Container 去处理他们的进来的请求。
 * 这个约定遵循，如：一个 non-SSL(非套接字) 和 SSL(套接字) 连接器共享相同的 web app 群。
 *
 * <p>
 ...
 *
 * 一个被给的 JVM 可包含任意数量的 Service 实例；不过，他们是完全独立于每个的，且仅仅共享
 * 系统中基本的JVM环境和类。
 *
